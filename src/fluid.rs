...
impl<'a, const M: usize> Grid<'a, M> {
    pub fn new(vx_grid: &'a mut [f32; M], vy_grid: &'a mut [f32; M], vz_grid: &'a mut [f32; M]) -> Self {
        // Velocity grids
        Self {
            x_vel: vx_grid,
            y_vel: vy_grid,
    ...
    pub fn get_index(&self, pos: Pos)-> usize {
        return pos.x + X_SIZE*(pos.y+ Y_SIZE * pos.z);
    }
...
pub fn step_fluid<const N: usize>(
    ...
    step_vel(grid.x_vel, grid.y_vel, TEX_SIZE as u32, dt, diff, borders);
}
